...
const COLORS = ['#4ecba3', '#7aa2f7', '#f6c177', '#e07070', '#b48ead'];
    ...
    <div className="border border-border bg-surface rounded p-4 flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="flex items-center justify-between gap-2 mt-2 text-xs">
          ...
          <span className={complete ? 'text-[#4ecba3]' : 'text-muted'}>
    ...
    <Modal open={open} onClose={onClose} title={goal ? 'Edit Goal' : 'New Goal'}>
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            className="flex items-center gap-1.5 px-4 py-2 text-xs bg-[#4ecba3] text-[#0f0f11] font-medium rounded hover:opacity-90"
            onClick={submit}
          >
            <Target size={13} />
            Save
    ...
    <div className="p-6 space-y-6">
      ...
      {goals.length > 0 && (
        <div className="border border-border bg-surface rounded p-4">
          ...
          <div className="h-2 bg-background border border-border rounded overflow-hidden">
            <div className="h-full bg-[#4ecba3]" style={{ width: `${totals.percent}%` }} />
          </div>
        </div>
      )}
